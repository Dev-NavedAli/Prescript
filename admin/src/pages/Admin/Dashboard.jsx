        ...
        <div className='pt-4 border border-t-0'>
          {
            dashData.latest_apointments.map((item, index) => (
              <div key={index} className='flex items-center px-6 py-3 gap-3 hover:bg-gray-100' >
                ...
                {
                item.cancelled ? <p className='text-red-400 font-medium'>Cancelled</p> : <img onClick={()=>cancelApointment(item._id)} className='w-10 cursor-pointer' src={assets.cancel_icon} alt="" /> 
                }
              </div>
            ))
          }
